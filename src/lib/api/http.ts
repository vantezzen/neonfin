...
export async function authenticate(
  ...
  if (!token) {
    return {
      error: apiError(401, "missing_bearer_token", "Missing bearer token"),
    };
  }
  const resolved = await resolveApiKey(token);
  if (!resolved) {
    return {
      error: apiError(401, "invalid_api_key", "Invalid or revoked API key"),
    };
  }
  ...
  if (!project) {
    return { error: apiError(401, "invalid_api_key", "Project not found") };
  }
  ...
  if (resolved.kind === "publishable") {
    if (!isOriginAllowed(project, origin)) {
      return { error: apiError(403, "origin_not_allowed", "Origin not allowed") };
    }
    if (opts.rateLimit !== false) {
      ...
      if (!rl.ok) {
        ...
        return {
          error: apiError(429, "rate_limited", "Rate limit exceeded", {
            ...cors,
