...
Use this when a missing wallet should remain missing (for example, an account
status check):
...
It returns the same wallet shape as the create-or-read endpoint, or `404` with
...
The response has `entries` and an opaque `nextCursor`. Each entry includes the
