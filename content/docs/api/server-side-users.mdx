...
This is the endpoint reference. For the mental model and worked examples
(better-auth, Clerk, Auth.js), see [External auth](/docs/concepts/external-auth).
The [`neonfin-server`](/docs/concepts/external-auth#the-server-client) registry
client is a typed wrapper around everything below.
...
await neonfin.revokeFeature("user_123", "analytics");
```
