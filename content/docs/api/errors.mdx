...
`PayError.requestId` is available when the API returned a request identifier.
Include it when reporting an unexpected provider or server error.

Every API error also has a `requestId` in its JSON body and `X-Request-Id`
response header. Include it when reporting an unexpected provider or server
error.
