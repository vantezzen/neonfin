...
Every API error has a `requestId` in its JSON body and `X-Request-Id` response
header. The typed client exposes it as `PayError.requestId`; include it when
reporting an unexpected provider or server error.
