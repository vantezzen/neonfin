...
![neonFin Dashboard](./dashboard.png)

Adding payments to a project usually takes a ton of work: Stripe or Polar
integration, checkout and webhook plumbing, user handling, billing UI. neonFin
...
project into the same system. neonFin doesn't replace Stripe or Polar; it sits
...
Stripe and Polar are excellent payment providers. neonFin does not replace
...
neonFin, Stripe or Polar still handle money, invoices, taxes, and payment
methods. Your app talks to one small neonFin integration.
...
2. neonFin creates a credit wallet and stores a code like `SKIP-8F3K-L9PQ-2MVT`.
3. Your code simply deducts minutes when a file is processed using the neonFin SDK.
...
6. neonFin receives the webhook and adds credits to the same wallet.
