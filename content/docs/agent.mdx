...
If you are a human developer, read the [getting started guide](/docs/getting-started) instead.
...
projects need the server SDK and small same-origin bridge routes for the React
components:
...
In external-auth mode, `WalletButton` does not show recovery codes or QR
controls. It becomes a billing-management entry point for the signed-in account.
...
When deducting credits, it's best to pass a stable `idempotencyKey` for retryable work, such as a job id or request id. If you omit it, the client generates one, which is fine for a single button click but not for server retries.
...
import { PayError, createPayServerClient } from "@/lib/pay/server";

const payServer = createPayServerClient({
  baseUrl: process.env.NEXT_PUBLIC_PAY_URL!,
  secretKey: process.env.PAY_SECRET_KEY!, // pay_sk_… - server only
});

export async function POST() {
  // Read this from your application's authenticated session.
  const userId = "your-stable-user-id";
  const jobId = crypto.randomUUID();
  ...
  try {
    ...
    if (err instanceof PayError && err.isInsufficientCredits) {
      ...
      return Response.json({ needsPayment: url }, { status: 402 });
    }
    throw err;
  }
  // 3. Run the paid work here.
...
async function runAnalyticsExport() {
  // Run the protected work here.
  return { ok: true };
}

export async function POST() {
  const userId = "your-stable-user-id";
  if (!(await payServer.hasFeature(userId, "analytics"))) {
    return Response.json({ error: "Analytics requires a paid plan" }, { status: 403 });
  }
  return Response.json(await runAnalyticsExport());
}
```
...
5. Add a price, pack, or subscription tier matching the code:
...
6. Open the project **Developers** tab.
7. Copy `NEXT_PUBLIC_PAY_URL` and `NEXT_PUBLIC_PAY_KEY` into the app env file,
...
8. Restart the app process if its framework requires env variables to be loaded
   at startup.
