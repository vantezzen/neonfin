...
| `allowPromotionCodes` | `boolean` | `false` | Lets customers enter a provider promotion or discount code at checkout. |
...
discountCode })`, which stores the order id in `localStorage`.
