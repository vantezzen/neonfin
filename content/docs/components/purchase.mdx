...
| `description` | `string` | `Payment is handled by our secure checkout.` | Dialog description. |
| `emptyMessage` | `string` | `Nothing is available to buy right now. Please check back later.` | Text shown when filters match no prices. |
...
| `recommendedPriceId` | `string` | - | Highlights one price as the suggested option. |
...
The dialog calls `client.startCheckout(priceId, { flow, allowPromotionCodes,
discountCode, customerEmail })`, which stores the order id in `localStorage`.
